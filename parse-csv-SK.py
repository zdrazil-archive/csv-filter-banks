            ...
            if 14 < len(row_csv):
                ...
                # If it's not a number save it, because it's a header
                    ...
                    if value_csv >= money_value:
                        ...
                        try:
                            row_date_csv = datetime.datetime.strptime(input_string_csv, '%d.%m.%Y')
                        except ValueError:
                            continue
                        else:
                            if row_date_csv >= date_from and row_date_csv <= date_to:
...
def create_final_file(from_final_file: str) -> str:
    ...
    with open(from_final_file, newline='', encoding="utf-8") as from_final_file_csv:
        ...
        for row_final in records_reader_final:
                ...
                if 14 < len(row_final):
                    ...
                    row_array = [row_final[0], row_final[6], row_final[13], row_final[7]]
        ...
        if len(info_array) < 1:
            ...
            for row_final in info_array:
                ...
                temp_date = datetime.datetime.strptime(row_final[0], '%d.%m.%Y')
    ...
    print("\n Soubor byl vytvořen. Jmenuje se PLATBY-SK.TXT a je ve složce \n"
